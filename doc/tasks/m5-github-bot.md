...
- [ ] **PR-05-01** — 实现 `app/bot/event_router.py` — `EventRouter`
  ...
  - 其他事件 → 200 忽略
- [ ] **PR-05-02** — 在 `app/main.py` 注册 `POST /webhook/github` 路由
- [ ] **PR-05-03** — 编写事件路由测试（各事件类型路由正确 + 签名校验失败返回 401）

### 5.2 自动评审处理器

- [ ] **PR-05-04** — 实现 `app/bot/auto_review.py` — `AutoReviewHandler`
...
- [ ] **PR-05-06** — 编写自动评审测试（Mock M2 + M4 → 验证完整调用链 + 异常恢复不崩溃）

### 5.3 命令处理器

- [ ] **PR-05-07** — 实现 `app/bot/command_handler.py` — `CommandHandler`
...
- [ ] **PR-05-08** — 编写命令解析测试
  ...
  - 普通评论 "looks good" → 忽略
- [ ] **PR-05-09** — 编写权限校验测试（协作者可触发 + 非协作者拒绝）

### 5.4 评论创建器

- [ ] **PR-05-10** — 实现 `app/bot/comment_creator.py` — `CommentCreator`
...
- [ ] **PR-05-11** — 实现 diff hunk 行号定位逻辑
  - 新文件行号 → diff hunk 行号映射
  - 无法定位时降级为 PR 对话评论
- [ ] **PR-05-12** — 编写评论格式化测试（输入 ReviewIssue → 验证 Markdown 结构 + emoji + diff 格式）

---

## 完成标准

- [ ] Webhook 事件正确路由到对应处理器
- [ ] PR 自动评审全流程串联（Webhook → M2 → M4 → M2 评论）
- [ ] `/review` 命令正确解析并触发分析
- [ ] 非协作者无法触发 `/review`
- [ ] 行级评论准确定位到 diff hunk 位置
- [ ] 总结评论包含完整的问题统计
- [ ] 异常不导致崩溃，返回 200 给 GitHub
